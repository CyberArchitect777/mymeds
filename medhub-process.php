...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if ($_POST["token"] == $_SESSION["medhub-token"]) { // Confirm the request came from the right form
            $medbox_id = $_POST["medboxid"];
            if ($_POST["medboxbutton"] == "Medicine Taken") {
                // Code for handling the medicine taken button
